...
if (sizeof($pk_mas)){
	...
	if ($cur_page>1){
			echo '<li class="page-item"><a href="#" class="page-link click-page-map" id='.($cur_page-1).'>Предыдущая</a></li>';
	};
	while ($page++ < $num_pages) {
		
		if($page == $cur_page){
			echo '<li class="page-item disabled">'.'<a href="#" class="page-link click-page-map" id='.$page.'>'.$page.'</a></li>';
		} else {
			echo '<li class="page-item">'.'<a href="#" class="page-link click-page-map" id='.$page.'>'.$page.'</a></li>';
		}
	}
	if ($cur_page<$num_pages){
			echo '<li class="page-item"><a href="#" class="page-link click-page-map" id='.($cur_page+1).'>Следующая</a></li>';
	};
}
...
if (sizeof($pk_mas)){
	...
	foreach($city_mas as $city) {
	    echo  "ymaps.geocode('$city',{ results: 1 }).then(
	        function (res) {
